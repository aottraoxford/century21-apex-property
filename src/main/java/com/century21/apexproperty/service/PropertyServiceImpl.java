...
public class PropertyServiceImpl implements PropertyService {
    ...
    public void updateStatus(int propertyID, boolean status, Principal principal, HttpServletRequest httpServletRequest) {
        if (propertyRepo.findAdminID(propertyID) == null)
            throw new CustomRuntimeException(401, "You are not the owner of the property");
    ...
    public List<PropertyRepo.Properties> findAgentProperties(int userID, String status, Pagination pagination, PropertyRepo.AgentPropertiesStatistic agentPropertiesStatistic) {
        ...
        agentPropertiesStatistic.setEnable(enable);
        agentPropertiesStatistic.setDisable(disable);
        agentPropertiesStatistic.setTotal(enable + disable);
        return properties;
    }

    @Override
    public PropertyRepo.Property updateProperty(PropertyRepo.PropertyUpdate property, Principal principal) {
        if (propertyRepo.isExist(property.getId()) == 0)
            throw new CustomRuntimeException(404, "Property id not found.");
        Integer userID = userRepo.findUserIDByEmail(principal.getName());
